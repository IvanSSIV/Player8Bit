...
static FSM_WORK FSM_badge;        // Macchina a stati per la gestione della logica di funzionamento dell'identificazione tramite badge
...
static BOOL auth_flag = FALSE;	            // Flag per determinare la differenza fra una scansione "normale" ed una per autorizzazione
static BOOL badge_auth_ena = FALSE;	    // Abilitazione del badge all'autorizzazione degli altri badge al prelievo di prodotti non abilitati
...
const char str_bdg_st_done[] PROGMEM         = { "ST_BADGE_DONE" };
...
const char str_bdg_ena_read[] PROGMEM        = { "* lettura badge abilitata *" };
const char str_bdg_disena_read[] PROGMEM     = { "* lettura badge disabilitata *" };
const char str_bdg_start_read[] PROGMEM      = { "inizio lettura badge" };
const char str_bdg_end_read[] PROGMEM        = { "fine lettura badge" };

// Vettore di program memory con i nomi degli stati
const char* const badge_state_name[] PROGMEM = {
	str_bdg_st_init,
	str_bdg_st_ready,
	str_bdg_st_reading,
	str_bdg_st_done,
...
void badge_init(void)
  {
     ...
     debug_message_CRLF();
     debug_message_timestamp_PGM(str_bdg_init_start);

     // inizializza la seriale del badge
     badge_init_LL(9600L);

	
     fsm_init(&FSM_badge,
	     ST_BADGE_INIT,
	     (void*)badge_state_name,
	     MAX_BADGE_STATE,
    	    (char *)"BDG");

     debug_message_timestamp_PGM(str_bdg_init_end);
}
...
void badge_ena_read(void)
  {
#ifdef DEBUG_VERSION
	 debug_print_timestamp(DBG_ALWAYS_ON, AVR_PGM_to_str(str_bdg_ena_read));
#endif
...
void badge_dis_read(void)
  {
#ifdef DEBUG_VERSION
   	 debug_print_timestamp(DBG_ALWAYS_ON, AVR_PGM_to_str(str_bdg_disena_read));
#endif
...
void badge_hand(void)
{
	...
	switch (FSM_badge.state)
	{
		...
		case ST_BADGE_READY:
		  fsm_first_scan(&FSM_badge, AVR_PGM_to_str(str_bdg_st_ready));
		...
		case ST_BADGE_READING:
...
#ifdef DEBUG_VERSION
				debug_print_timestamp(bdg_dbg, AVR_PGM_to_str(str_bdg_start_read));
#endif

				badge_read = FALSE;
				fsm_set_timer(&FSM_badge, FSM_TIMER1, 1000L);
		  ...
		  if (!fsm_check_end_time(&FSM_badge, FSM_TIMER1))
		    {
			   uint16_t temp_chr;
...
#ifdef DEBUG_VERSION
			   debug_print_timestamp(TRUE, AVR_PGM_to_str(str_bdg_end_read));
			   badge_print();
#endif
		...
		case ST_BADGE_DONE:
		  fsm_first_scan(&FSM_badge, AVR_PGM_to_str(str_bdg_st_done));
		  fsm_set_state(&FSM_badge, ST_BADGE_READY);
		  break;

		case ST_BADGE_ERROR:
			fsm_first_scan(&FSM_badge, AVR_PGM_to_str(str_bdg_st_error));
			fsm_set_state(&FSM_badge, ST_BADGE_READY);
			break;

		case ST_BADGE_FAIL:
			fsm_first_scan(&FSM_badge, AVR_PGM_to_str(str_bdg_st_fail));
			break;
