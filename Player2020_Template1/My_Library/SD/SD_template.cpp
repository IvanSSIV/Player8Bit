...
// Include librerie non-Arduino dependent
...
// Include librerie Arduino dependent
#include "Arduino.h"
#include <SPI.h>
#include <SD.h>
...
void SD_append(uint8_t sdfileindex, char* sdbuff)
  {
	char answbuff[50];
	if (SD.exists(SD_file_name[sdfileindex])) 
	  {
		 sprintf(answbuff, "Esiste il file %s", SD_file_name[sdfileindex]);
		 debug_print_timestamp(sd_dbg, (char*)answbuff);
		 data_file = SD.open(SD_file_name[sdfileindex], FILE_WRITE);
		 data_file.println(sdbuff);
		 data_file.close();
	  }
	else
	{
		sprintf(answbuff, "Non esiste il file %s", SD_file_name[sdfileindex]);
		debug_print_timestamp(sd_dbg, (char*)answbuff);
	}
  }
...
BOOL SD_read(uint8_t sdfileindex, char * sdbuff, uint32_t start, uint32_t end)
  {
	...
	if (SD.exists(SD_file_name[sdfileindex]))
	  {
		data_file = SD.open(SD_file_name[sdfileindex], FILE_READ);
			ret_val_read = TRUE;
...
BOOL SD_read_config(VM_DATA* vm)
  {
	 ...
	 if (SD.exists(SD_file_name[FILE_CONFIG]))
	   {
		 debug_print_timestamp(sd_dbg, (char*)"Inizio lettura config ...");
		  ret_val_config = TRUE;
		  vecbuff = 0;
		  memset(buff, 0, sizeof(buff));
		  data_file = SD.open(SD_file_name[FILE_CONFIG], FILE_READ);
		  debug_print_timestamp_ident(sd_dbg, 3, (char*)"Numero computer: ");
		  ...
		  vecbuff = 0;
		  debug_print_timestamp_ident(sd_dbg, 3, (char*)"Tipo computer: ");
		  ...
		  // Lettura IP
		  debug_print_timestamp_ident(sd_dbg, 3, (char*)"IP macchina: ");
		  ...
		  debug_print_timestamp_ident(sd_dbg, 3, (char*)"Subnet Mask: ");
		  ...
		  // Gateway
		  debug_print_timestamp_ident(sd_dbg, 3, (char*)"Gateway: ");
