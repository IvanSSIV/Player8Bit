...
const char str_mot_init_start[] PROGMEM = { "Inizializzazione della macchina a stati del motore." };
const char str_mot_init_end[] PROGMEM = { "Fine inizializzazione della macchina a stati del motore." };
...
{
	debug_print_timestamp_ident(DBG_ALWAYS_ON, DEBUG_IDENT_L1, AVR_PGM_to_str(str_mot_init_start));
	LL_motor_init();
	...
	debug_print_timestamp_ident(DBG_ALWAYS_ON, DEBUG_IDENT_L1, AVR_PGM_to_str(str_mot_init_end));
...
void motor_hand(void)
{
	...
	case ST_MOT_STARTING:
		...
		if (fsm_timer_hand(&FSM_motor, FSM_TIMER1, ritardo_taglio_motore))   // 300 ms di delay per attesa misurazione taglio corrente
		{
